...
export default function VideoPage() {
  ...
  const channelId = parseInt(pathname.slice(pathname.lastIndexOf("/") + 1));
  ...
  const getChannelInfo = async () => {
    if (!channelId) return;
    const res = await Fetch(`/api/channels/getFromId`, "post", {
      id: channelId,
    });
    ...
    const relatedVideosRes = await Fetch(
      `/api/videos/getRelatedFromChannel`,
      "post",
      { id: channelId }
    );
            ...
            <section className="relative flex flex-col lg:grid grid-cols-6">
              <div className="lg:col-span-4 flex flex-col gap-2 p-4 lg:p-8 lg:h-[calc(100dvh-80px)] lg:overflow-y-scroll pt-[100px]">
                ...
                <div className="rounded-xl bg-grey/30 p-4 flex flex-col gap-3">
                  <p className="text-left text-grey text-sm block">Live Now</p>
                  {/* <p className="text-left text-white text-sm block">{videoData.description}</p> */}
                </div>
              </div>
